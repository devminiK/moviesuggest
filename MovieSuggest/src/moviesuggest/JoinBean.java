...
public class JoinBean {
	...
	public Join getDB(Join join){
		...
		try{
			...
			//값을 넣음.
			join2.setUser_name(rs.getString("user_name"));
			join2.setUser_id(rs.getString("user_id"));
			join2.setUser_password(rs.getString("user_password"));
			join2.setUser_email(rs.getString("user_email"));
			join2.setUser_tel(rs.getString("user_tel"));
	...
	public ArrayList<Join> getDBList(){
		...
		try{
			...
			// 반환값을 가져옴. select ~를 받아서 처리하여 반환해줌. ResultSet으로 받아야 제대로 값을 받을 수가 있음.
